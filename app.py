...
# Store your API keys securely using Streamlit Secrets
...
def main_app():
    ...
    # -------------------- USER INPUT --------------------
    keyword = st.text_input("Enter keyword or hashtag to analyze:", "Nike")
    ...
    # -------------------- FETCH REAL TWEETS AND ANALYZE SENTIMENT --------------------
    @st.cache_data(show_spinner=True)
    def fetch_and_analyze_tweets(query, count):
        try:
            ...
            for tweet in tweets:
                ...
                data.append({
                    ...
                    "replies": tweet.in_reply_to_status_id_str if hasattr(tweet, 'in_reply_to_status_id_str') else 0,
    ...
    # -------------------- MAIN LOGIC --------------------
    if keyword:
        df = fetch_and_analyze_tweets(keyword, max_results)
        ...
        if df.empty:
            ...
            st.download_button(
                ...
                data=csv,
                file_name=f'filtered_tweet_data_{keyword.replace(" ", "_")}.csv',
                mime='text/csv',
            )
